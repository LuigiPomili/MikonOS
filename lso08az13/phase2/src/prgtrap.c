/****************************************************************************
 * Program Trap exception management
 ***************************************************************************/
...
/*
 * funzione attivata dall'HW quando si cerca di eseguire una azione
 * illegale o indefinita.
...
void trap_handler(void) {
	...
	/* salvataggio del contesto, contenuto nella PGMTRAP_OLDAREA, nel cpu_snapshot del caller */
	initCPUSnap(&(caller_t->cpu_snapshot), PGMTRAP_OLDAREA, FALSE);

	if (caller_t->prgtrap_m == FALSE){ /* gestore non presente, kill caller */
		...
		SYSMsgSend(caller, caller_t->prgtrap_m);
	}
	schedule();
}
